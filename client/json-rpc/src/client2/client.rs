...
impl Client {
    ...
    async fn send<T: DeserializeOwned>(&self, request: MethodRequest) -> Result<Response<T>> {
        ...
        if request.id() != id {
            return Err(Error::unexpected_invalid_response_id(resp));
        }

        let inner = serde_json::from_value(result)?;
    ...
    async fn send_impl<S: Serialize, T: DeserializeOwned>(&self, payload: &S) -> Result<T> {
        ...
        if response.status() != 200 {
            return Err(Error::InvalidHTTPStatusUreq(response.status().as_u16()));
        }

        response.json().await.map_err(Error::InvalidHTTPResponse)
    }
}
...
async fn call_metadata() {
    ...
    let client = Client::new("http://testnet.diem.com/v1");
    let _ = dbg!(client.get_metadata().await);
    let _ = dbg!(client.get_network_status().await);
}
