...
impl BlockingClient {
    ...
    fn send_impl<S: Serialize, T: DeserializeOwned>(&self, payload: &S) -> Result<T> {
        ...
        if resp.status() != 200 {
            return Err(Error::InvalidHTTPStatusUreq(resp.status()));
        }

        resp.into_json_deserialize().map_err(Into::into)
    }
}
...
fn call_metadata() {
    ...
    let _ = dbg!(client.get_metadata());
    let _ = dbg!(client.get_network_status());
}
