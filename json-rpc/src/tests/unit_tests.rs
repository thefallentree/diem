...
fn test_transaction_submission() {
    ...
    if let Err(diem_json_rpc_client::Error::JsonRpcError(error)) = response {
...
fn test_get_events_page_limit() {
    ...
    let ret = client.get_events("13000000000000000000000000000000000000000a550c18", 0, 1001);

    assert!(ret.is_err());
    let expected = "JsonRpcError(JsonRpcError { code: -32600, message: \"Invalid Request: page size = 1001, exceed limit 1000\", data: None })";
    assert_eq!(ret.unwrap_err().to_string(), expected)
}
...
fn test_get_transactions_page_limit() {
    ...
    let ret = client.get_transactions(0, 1001, false);
    assert!(ret.is_err());
    let expected = "JsonRpcError(JsonRpcError { code: -32600, message: \"Invalid Request: page size = 1001, exceed limit 1000\", data: None })";
    assert_eq!(ret.unwrap_err().to_string(), expected)
}
